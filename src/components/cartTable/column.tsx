...
export const cartColumns: ColumnDef<CartItem>[] = [
    {
        id: 'product',
        accessorFn: row =>
            ({ id: row.id, name: row.name, image: row.image }) satisfies ProductColumn,
        header: () => (
            <div className="">
                <h2 className="text-sm md:text-[16px]  font-poppins font-normal text-black ">
                    Product
                </h2>
            </div>
        ),
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-6">
                    <div className="relative group">
                        <img
                            src={image}
                            alt={name}
                            className="w-10 h-10 md:w-[3.25rem] md:h-[3.25rem] object-cover cursor-pointer"
                        />
                        <button
    ...
    },
    {
        id: 'quantity',
        accessorFn: row =>
            ({ id: row.id, quantity: row.quantity }) satisfies ProductQuantity,
        header: () => (
            <div>
                <h2 className="text-sm md:text-[16px] font-poppins font-normal text-black">
                    Quantity
